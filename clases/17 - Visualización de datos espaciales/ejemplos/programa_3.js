...
// Archivo obtenido tras ejecutar:
// geo2topo paises_con_densidad.json > paises-topo.json
// toposimplify -p 0.05 -f < paises-topo.json > paises-topo-simple.json

d3.json("paises-topo-simple.json").then((datos) => {
  const datosGeojson = topojson.feature(
    datos,
    datos.objects["paises_con_densidad"]
  );
  const tope = 500;
  ...
  const proyeccion = d3.geoWinkel3().fitSize([width, height], datosGeojson);
  const caminosGeo = d3.geoPath().projection(proyeccion);

  svg
    .selectAll("path")
    .data(datosGeojson.features)
    .enter()
    .append("path")
    .attr("d", caminosGeo)
